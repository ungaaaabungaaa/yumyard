...
function DeliveryViewOrderContent() {
  ...
  const handleMarkDelivered = async () => {
    if (!orderId || !orderWithUser) return;

    setIsUpdating(true);
    try {
      await updateOrderStatus({
        id: orderId as Id<"orders">,
        status: "delivered" as "order-received" | "cooking" | "out-for-delivery" | "delivered" | "cancelled",
      ...
      <div className="fixed bottom-4 left-4 right-4 max-w-full py-4 z-40">
        <button
          ...
          className={`w-full py-6 px-8 rounded-2xl text-lg font-bold transition-colors duration-200 ${
            isUpdating || order.status !== "out-for-delivery"
...
export default function DeliveryViewOrder() {
  return (
    <Suspense fallback={

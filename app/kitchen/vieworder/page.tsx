...
function ViewOrderContent() {
  ...
  const handleDoneCooking = async () => {
    ...
    setIsUpdating(true);
    try {
      // Determine the next status based on order type
      const nextStatus = orderWithUser.orderType === "delivery" 
        ? "out-for-delivery" 
        : "delivered";

      await updateOrderStatus({
        id: orderId as Id<"orders">,
        status: nextStatus as "order-received" | "cooking" | "out-for-delivery" | "delivered" | "cancelled",
        staffName: "Kitchen Staff",
        note: "Order completed cooking",
      });
    } catch (error) {
